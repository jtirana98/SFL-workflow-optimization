...
def main():
    ...
    f = cp.Variable(K)
    f_par = cp.Parameter((K))
    f_int = cp.Variable((K), integer=True)
    f_int_par = cp.Parameter((K))
    ...
    f_int_par.value = np.zeros((K)).astype(int)
    f_par.value = np.zeros((K)).astype(int)
    ...
    obj_cont = cp.Minimize(cp.max(f + proc_local + cp.hstack(trans))
                ...
                + (rho/2)*cp.sum(cp.abs(f - f_int_par + nu)))
    ...
    obj_int = cp.Minimize((rho/2)*cp.sum(rhosum_int)
            ...
            + (rho/2)*cp.sum(cp.abs(f_int - f_par + nu)))
    ...
    for iter in range(MAX_ITER):
        ...
        prob_int.solve(solver=cp.GUROBI, verbose=False)
        ...
        if iter==0:
            ...
            aaa = LA.norm((y_par-y.value), 'fro')**2
