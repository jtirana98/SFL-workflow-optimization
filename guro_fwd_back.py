        ...
        if iter == 3: #stable >3 or iter == MAX_ITER - 1:
            ...
            for i in range(H):
                ...
                if back_flag: # Call sub-problems for back  
                    ...
                    Tz = np.max(release_datez) + len(Kx)*np.max(procz) + len(Kx)*np.max(procx) + np.max(proc_localx) + np.max(trans_backx) 
                    print(Tz)
                    start_sub = time.time()
                    (x__, z__) = sub.for_each_machine_fwd_back(len(Kx), release_datex, procx, proc_localx, trans_backx, release_datez, procz, proc_localz, trans_backz, memory_capacity[i], Tz)
                    end_sub = time.time()
                    machine_time = end_sub - start_sub
