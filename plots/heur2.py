...
admm = resnet_1["ADMM"]
baseline = resnet_1["baseline"]
load = resnet_1["balanced-greedy"]
for j in range(0, len(x)):
    axs[0,0].plot([j, j], [admm[j], baseline[j]], color='black', linestyle = 'dashed', linewidth=1)
    axs[0][0].annotate('', xy=(j,3+baseline[j]), xytext=(j+0.6,3+baseline[j]), arrowprops=props)
    #axs[0,0].plot([j, j+0.6], [5+baseline[j], 5+baseline[j]], color='black', linewidth=1.5, marker='|')
    axs[0][0].annotate(factor[j], xy=(j,10+baseline[j]), zorder=20, fontsize=14)
...
admm = vgg_1["ADMM"]
baseline = vgg_1["baseline"]
load = vgg_1["balanced-greedy"]
for j in range(0, len(x)):
    max_v = max([baseline[j], admm[j], load[j]])
    if j < len(x)-4:
        ...
        axs[0][1].annotate(factor[j], xy=(j,7+max_v), zorder=11, fontsize=14)
    else:
        ...
        axs[0][1].annotate(factor[j], xy=(j+0.3,7+max_v), zorder=11, fontsize=14)
multiplier = 0
iter = 0
...
for j in range(0, len(x)):
    ...
    axs[1][0].annotate(factor[j], xy=(j,30+max_v), zorder=11, fontsize=14)
...
for j in range(0, len(x)):
    ...
    axs[1][1].annotate(factor[j], xy=(j,30+max_v), zorder=11, fontsize=14)    
